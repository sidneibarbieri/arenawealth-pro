...
| The fee-aware planner matches the MIP deployment optimum and the fee lower bound in its design regime. | `make experiments` and the `planner_optimality` JSON block under `exports/` | Supported on the small-cash grid; the MIP deploys more only once a concentration limit binds |
