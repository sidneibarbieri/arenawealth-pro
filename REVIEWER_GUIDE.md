...
| Main contribution | Admissibility is an independent property; on real frontier models the dominant failure is **computation, not judgment**, which a deterministic layer removes. |
...
to the value claimed in the paper, plus `truncated=0` and a data-integrity hash
check. If every line says PASS, the headline experiment reproduced.
...
|---|---|---|---|
| Frontier gradient (Sec. 3) | arithmetic-not-judgment | `make review` | console PASS lines |
...
| Table 2 (scenario bank) | agreement-only FP = 0.42 | `make ai-advisor-audit` | console + experiment JSON |
